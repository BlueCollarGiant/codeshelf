...
router.post('/repos/visibility', async (req, res, next) => {
  try {
    ...
    const ownerLogin = await getAuthenticatedOwnerLogin();
    const results = await setVisibilityForAll(repos, ownerLogin);
    res.json({ results });
  } catch (err) {
    next(err);
  }
...
router.post('/repos/delete', async (req, res, next) => {
  try {
    ...
    for (const item of repos) {
      if (typeof item.fullName !== 'string' || item.fullName.split('/').length !== 2) {
