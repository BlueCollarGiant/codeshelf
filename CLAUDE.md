...
- **Classification runs before scoring.** `classifyRepo()` assigns a repo type (profile repo, portfolio project, fork, template, config/dotfiles, archived, experiment, and so on), then a type-specific scorer produces portfolio, cleanup, activity, and completeness scores plus suggestions.
...
2. Setup screen explains `.env` requirements and links to GitHub token settings. It never shows a token input and never displays the token value
3. Dashboard loads all repos via the local backend, with Public and Private sections as cards with checkboxes
...
12. Do not use em dashes in copy, prompts, docs, or comments. Use periods, colons, commas, semicolons, or parentheses instead.
